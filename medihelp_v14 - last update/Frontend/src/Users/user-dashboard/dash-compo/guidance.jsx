        ...
        <div className="bg-card dark:bg-card border-t border-b border-border/50 dark:border-border/30 rounded-lg p-6">
            ...
            <div className="space-y-4">
                {recentGuidance?.map((item) =>
                    <div
                        key={item?.id}
                        className={`border-l-4 ${getSeverityColor(item?.severity)} pl-4 py-3 px-4 bg-background hover:bg-primary 
                        rounded-r-lg transition-all duration-200 hover:scale-[1.02] text-foreground cursor-pointer 
                        hover:text-primary-foreground shadow-sm`}
                    >
