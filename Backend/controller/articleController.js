...
exports.recordArticleVisit = async (req, res) => {
    ...
    try {
        // 1. Check kung existing na ang history
        const [existing] = await db.execute(
            'SELECT * FROM user_reading_history WHERE user_id = ? AND article_id = ?',
            [userId, articleId]
        );

        if (existing.length > 0) {
            // 2. Kung mayroon na, i-update lang (at i-unarchive kung kinakailangan)
            await db.execute(
                'UPDATE user_reading_history SET last_visited = CURRENT_TIMESTAMP, is_archived = 0 WHERE user_id = ? AND article_id = ?',
                [userId, articleId]
            );
        } else {
            // 3. KUNG WALA PA, DITO MAG-IINSERT
            await db.execute(
                'INSERT INTO user_reading_history (user_id, article_id, last_visited, progress_percentage, is_archived) VALUES (?, ?, CURRENT_TIMESTAMP, 0, 0)',
                [userId, articleId]
            );
        }

        // 4. Sabay update ng view count sa articles table
        await db.execute(
...
exports.getArchivedHistory = async (req, res) => {
    ...
    try {
        const query = `
            ...
            INNER JOIN article_categories ac ON ga.cat_id = ac.category_id
            WHERE arh.UserID = ?
...
exports.updateReadingProgress = async (req, res) => {
    ...
    try {
        const query = `
            ...
            SET progress_percentage = GREATEST(progress_percentage, ?), 
                last_visited = CURRENT_TIMESTAMP
            WHERE user_id = ? AND article_id = ? AND is_archived = 0
        `;
    ...
    try {
        const query = `
            SELECT 
                sl.saved_date, 
                sl.saved_time,
                CONCAT(sl.saved_date, ' ', sl.saved_time) as full_saved_at, 
                ga.*, 
                ac.category_name
...
// Idagdag mo ito sa articleController.js
exports.submitArticleFeedback = async (req, res) => {
    const userId = req.user.UserID;
    const { article_id, is_helpful } = req.body;

    if (!article_id || is_helpful === undefined) {
        return res.status(400).json({ message: "Article ID and feedback status are required." });
    }

    try {
        // SQL query na gumagamit ng CURDATE() at CURTIME()
        const query = `
            INSERT INTO article_feedback (article_id, user_id, is_helpful, vote_date, vote_time)
            VALUES (?, ?, ?, CURDATE(), CURTIME())
        ...
        const [result] = await db.query(query, [article_id, userId, is_helpful]);

        res.status(200).json({ 
            success: true, 
            message: "Feedback submitted successfully!",
            // Para malaman kung nag-insert (1) o nag-update (2)
...
exports.getFeedbackStatus = async (req, res) => {
    ...
    try {
        const [rows] = await db.query(
            'SELECT is_helpful FROM article_feedback WHERE user_id = ? AND article_id = ?',
            [userId, articleId]
        );

        if (rows.length > 0) {
            res.json({ hasFeedback: true, is_helpful: rows[0].is_helpful });
        } else {
