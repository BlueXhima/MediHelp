...
CREATE TABLE `medical_conditions` (
  ...
  `Status` enum('Active','Recovered','Under Treatment') DEFAULT 'Active',
  `Created_Date` date DEFAULT (curdate()),
  `Created_Time` time DEFAULT (curtime()),
  `Updated_Date` date DEFAULT NULL,
  `Updated_Time` time DEFAULT NULL,
  PRIMARY KEY (`ConditionID`),
  KEY `fk_conditions_patient` (`PatientID`),
  CONSTRAINT `fk_conditions_patient` FOREIGN KEY (`PatientID`) REFERENCES `patient` (`PatientID`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `medical_conditions` VALUES (23,2,'test','2026-05-01','Recovered','2026-05-06','16:03:54',NULL,NULL);
