//profile.jsx - Main component for the patient's health profile dashboard, including tabs for personal info, medical records, medications, emergency contacts, metrics, and privacy settings.
        ...
        const fetchUserDetails = async () => {
            ...
            try {
                ...
                if (userResponse.data) {
                    combinedData = {
                        ...combinedData,
                        userID: userResponse.data.userID || userResponse.data.userId || userResponse.data.UserID,
                        firstName: userResponse.data.firstName || '',
                        lastName: userResponse.data.lastName || '',
                        email: userResponse.data.email || storedEmail,
                ...
                if (combinedData.userID) {
                    try {
                        const patientResponse = await axios.get(
                            `http://localhost:5000/api/users/patient-profile/${combinedData.userID}`
                        );
                        ...
                        if (patientResponse.data) {
                            combinedData = {
                                ...combinedData,
                                firstName: patientResponse.data.FirstName || combinedData.firstName,
                                lastName: patientResponse.data.LastName || combinedData.lastName,
                                address: patientResponse.data.Address || '',
                                gender: patientResponse.data.Gender || '',
                                height: patientResponse.data.Height_cm || '',
                                weight: patientResponse.data.Weight_kg || '',
                                dob: patientResponse.data.DateOfBirth || '',
                                bloodType: patientResponse.data.BloodType || '',
                                patientID: patientResponse.data.PatientID || null,
                                email: combinedData.email,
                                userID: combinedData.userID,
                                profile_picture: combinedData.profile_picture
                            };
                        }
                    } catch (patientError) {
                        console.log("No patient record found yet:", patientError.message);
                    }
                }
            ...
            <main className="max-w-5xl mx-auto px-6 mt-10">
                <div className="bg-card border border-slate-200 rounded-[40px] shadow-sm p-8 md:p-12 min-h-[500px] animate-fade-in relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        {activeTab === 'records' && (
                            <MedicalRecord isLoading={isLoading} patientId={userData.patientID} />
                        )}

                        {activeTab === 'medications' && (
                            <Medications isLoading={isLoading} patientId={userData.patientID} />
                        )}

                        {activeTab === 'emergency' && (
                            <Emergency isLoading={isLoading} patientId={userData.patientID} />
                        )}

                        {activeTab === 'metrics' && (
                            <Metrics isLoading={isLoading} patientId={userData.patientID} />
                        )}
