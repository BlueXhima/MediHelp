...
import { Plus, X, Trash2, Edit3, Loader, AlertCircle, Search } from 'lucide-react';
import { articleService } from '../../services/articleService'; // Import mula sa articleService
...
// 2. Glossary Form Component
const GlossaryForm = ({ initialData, onSubmit, onCancel, categories }) => {
  const [formData, setFormData] = useState(initialData || { term: '', definition: '', category_name: '' });
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea required className="w-full p-2 border border-[hsl(var(--border))] rounded-lg h-24" 
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button type="submit" className="px-4 py-2 bg-[hsl(var(--primary))] text-white rounded-lg">Save Term</button>
        </div>
      </div>
    </form>
  );
...
// 3. Main GlossaryManage Component
const GlossaryManage = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      // Fetching categories from articleService
      const categoriesData = await articleService.getCategories();
      setCategories(categoriesData || []);
      
      // Dito mo i-fetch ang actual glossary items mo
      // setGlossary(await glossaryService.getAll());
    } catch (error) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[hsl(var(--foreground))]">Glossary Management</h2>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Manage terminology and definitions.</p>
        </div>
      ...
      <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-[hsl(var(--border))] flex flex-wrap items-center gap-4">
            <div className="relative flex-1 max-w-sm">
                ...
                <input type="text" placeholder="Search terms..." className="w-full pl-10 pr-4 py-2 bg-[hsl(var(--muted))] border-none rounded-lg" onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-[hsl(var(--muted-foreground))]"><AlertCircle className="mx-auto mb-2 opacity-20" /> No terms found.</div>
        ) : (
            ...
            <tbody className="divide-y divide-[hsl(var(--border))]">
              {filteredGlossary.map((item) => (
                <tr key={item.id} className="hover:bg-[hsl(var(--muted))/30]">
                  <td className="px-6 py-4 font-semibold text-[hsl(var(--primary))]">{item.term}</td>
                  <td className="px-6 py-4">{item.category_name}</td>
                  <td className="px-6 py-4 text-[hsl(var(--muted-foreground))]">{item.definition}</td>
      ...
      <Modal isOpen={isModalOpen} title="Add New Glossary Term" onClose={() => setIsModalOpen(false)}>
        <GlossaryForm 
