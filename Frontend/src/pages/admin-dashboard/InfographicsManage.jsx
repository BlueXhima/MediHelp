...
import { Plus, X, Trash2, Edit3, Loader, AlertCircle, Search, ImageIcon } from 'lucide-react';
...
// 2. Infographic Form Component
const InfographicForm = ({ initialData, onSubmit, onCancel, categories }) => {
  const [formData, setFormData] = useState(initialData || { title: '', category_name: '', imageUrl: '' });
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }}>
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button type="submit" className="px-4 py-2 bg-[hsl(var(--primary))] text-white rounded-lg">Save Infographic</button>
        </div>
      </div>
    </form>
  );
...
// 3. Main InfographicsManage Component
const InfographicsManage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Dito mo i-fetch ang listahan ng infographics
      // setInfographics(await infographicService.getAll());
      setLoading(false);
    } catch (error) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[hsl(var(--foreground))]">Infographics Management</h2>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Manage visual educational content.</p>
        </div>
      ...
      <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-[hsl(var(--border))] flex flex-wrap items-center gap-4">
            <div className="relative flex-1 max-w-sm">
                ...
                <input type="text" placeholder="Search infographics..." className="w-full pl-10 pr-4 py-2 bg-[hsl(var(--muted))] border-none rounded-lg" onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-[hsl(var(--muted-foreground))]"><AlertCircle className="mx-auto mb-2 opacity-20" /> No infographics found.</div>
        ) : (
            ...
            <tbody className="divide-y divide-[hsl(var(--border))]">
              {filteredInfographics.map((item) => (
                <tr key={item.id} className="hover:bg-[hsl(var(--muted))/30]">
                  <td className="px-6 py-4 flex items-center gap-3 font-medium"><ImageIcon size={20} className="text-[hsl(var(--primary))]" /> {item.title}</td>
      ...
      <Modal isOpen={isModalOpen} title="Add New Infographic" onClose={() => setIsModalOpen(false)}>
        <InfographicForm 
...
export default InfographicsManage;